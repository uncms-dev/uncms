...
* `BLEACH_OPTIONS` was never validated; if you had an invalid configuration you would get an exception at runtime, where it might not be tested. The replacement `NH3_OPTIONS` is now validated with a system check, because that happens immediately on startup, and is almost free to run.
