...
def test_redirectadmin_change(admin_client):
    ...
    response = admin_client.get(
        reverse("admin:redirects_redirect_change", args=[redirect.pk])
    )
    assert response.status_code == 200
...
def test_redirectadmin_list(admin_client):
    ...
    response = admin_client.get(reverse("admin:redirects_redirect_changelist"))
    assert response.status_code == 200
    assert response.context_data["cl"].list_display == [
        "action_checkbox",
        "old_path",
        "new_path",
        "type",
...
@pytest.mark.parametrize(
    "user_permissions, expect_enabled",
    [
        ([], False),
...
def test_redirectadmin_import_csv_view(client):
    ...
    assert (
        submit_button_text(BeautifulSoup(response.content, "html.parser"))
        == "Check file"
    )
