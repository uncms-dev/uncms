...
def test_importredirectscsv_dry_run():
    ...
    call_command(
        "import_redirects_csv", data_file_path("redirects_no_header.csv"), "--dry-run"
    )
...
def test_importredirectscsv_ignore_errors():
    ...
    with pytest.raises(CommandError) as exc:
        call_command(
            "import_redirects_csv", data_file_path("redirects_with_header.csv")
        )
    ...
    call_command(
        ...
        data_file_path("redirects_with_header.csv"),
        "--ignore-errors",
    )
    assert Redirect.objects.count() == 2
...
def test_importredirectscsv_verbosity():
    ...
    call_command(
        "import_redirects_csv",
        data_file_path("redirects_no_header.csv"),
        "--verbosity=2",
    )
    assert Redirect.objects.count() == 2
