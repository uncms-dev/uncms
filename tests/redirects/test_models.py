...
def test_redirectmanager_get_queryset():
    Redirect.objects.create(
        old_path="/sample/", new_path="/example/", regular_expression=False
    )
    Redirect.objects.create(
        old_path="/sample/(.*)/", new_path="/example/", regular_expression=True
    )
...
def test_redirect_clean():
    ...
    redirect = make_redirect(
        {"new_path": "https://wat.example.invalid/hello/?query=1#fragment"}
    )
    redirect.clean()
...
def test_redirect_permanent():
    ...
    assert (
        Redirect(
            old_path="/one/", new_path="/two/", type=RedirectTypeChoices.TEMPORARY
        ).permanent
        is False
    )
...
def test_response_response_for_path():
    ...
    temporary = Redirect(
        old_path="/sample/", new_path="/example/", type=RedirectTypeChoices.TEMPORARY
    )
    ...
    temporary = Redirect(
        old_path="/sample/", new_path="/example/", type=RedirectTypeChoices.PERMANENT
    )
    response = temporary.response_for_path("/sample/")
...
def test_redirect_str():
    ...
    assert (
        str(Redirect(old_path="/stringme/", new_path="/stringme2/"))
        == "/stringme/ 🡢 /stringme2/"
    )
