...
class File(models.Model):
    ...
    def get_thumbnail(
        ...
        *,
        ...
        url = reverse(
            f"{defaults.MEDIA_URLS_NAMESPACE}:image_view",
            kwargs={
                "pk": self.pk,
                "width": str(width),
                "height": str(height),
        ...
        if width != "auto" and height != "auto":
            ...
            thumb_width, thumb_height = width, round(
                self.height * (width / (self.width or 1))
            )
        else:
            # due to the exception raised if both are 'auto', this is the only
            # other option: height specified, width auto
            thumb_width, thumb_height = (
                round(self.width * (height / (self.height or 1))),
                height,
            )
