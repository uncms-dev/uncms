...
def _navigation_entries(context, pages, section=None, json_safe=False):
    ...
    if section:
        ...
        section_entry["here"] = (
            context["request"].pages.current == section_entry["page"]
        )
...
def render_navigation(context, pages, section=None, class_prefix=None, **templates):
    ...
    return {
        ...
        "item_template": templates.get(
            "item_template", defaults.NAVIGATION_ITEM_TEMPLATE
        ),
        "submenu_template": templates.get(
            "submenu_template", defaults.NAVIGATION_SUBMENU_TEMPLATE
        ),
        "submenu_item_template": templates.get(
            "submenu_item_template", defaults.NAVIGATION_SUBMENU_ITEM_TEMPLATE
        ),
    }


def get_breadcrumbs_obj(
...
def render_title(context, browser_title=None):
    ...
    return {
        ...
        or "",
        "site_title": (homepage and homepage.browser_title)
        or (homepage and homepage.title)
        or "",
    }
