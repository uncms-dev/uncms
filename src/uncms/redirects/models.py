...
class Redirect(models.Model):
    ...
    old_path = models.CharField(
        ...
        unique=True,
        help_text=_(
            'This can either be a whole URL, or just a path excluding the domain name (starting with "/"); it will be normalised to a path when you save. Example: "https://www.example.com/events/" or "/events/".'
        ),
    )
    ...
    new_path = models.CharField(
        ...
        blank=True,
        help_text=_(
            "If you leave this empty, a 410 Gone response will be served for this URL."
        ),
    )
    ...
    test_path = models.CharField(
        ...
        blank=True,
        help_text=_(
            "You will need to specify a test path to ensure your regular expression is valid."
        ),
    )
    ...
    def clean(self):
        ...
        if self.regular_expression:
            ...
            try:
                ...
                raise ValidationError(
                    {
                        "new_path": _(
                            "There was an error in your substitution: {}"
                        ).format(str(err))
                    }
                ) from err
        else:
            ...
            if not self.old_path.startswith("/"):
                raise ValidationError(
                    {
                        "old_path": _(
                            '"From" path must either be a full URL or start with a forward slash.'
                        ),
                    }
                )
            ...
            if (
                ...
                and parsed.netloc in settings.ALLOWED_HOSTS
            ):
        ...
        if self.old_path == self.new_path:
            raise ValidationError(
                {
                    "old_path": _(
                        "New path must not be the same as the old one (you can't redirect something to itself)."
                    ),
                }
            )
